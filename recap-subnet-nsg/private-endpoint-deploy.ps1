...
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ERROR] OpenAI service $openAIName not found. Please deploy it first." -ForegroundColor Red
    exit 1
}
Write-Host "[SUCCESS] OpenAI service verified" -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ERROR] Subnet $subnetName not found. Please create networking infrastructure first." -ForegroundColor Red
    exit 1
}
Write-Host "[SUCCESS] Private endpoint subnet verified" -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "[WARNING] Private endpoint $privateEndpointName already exists. Skipping creation." -ForegroundColor Yellow
    ...
    Write-Host "[SUCCESS] Existing Private Endpoint Details:" -ForegroundColor Green
...
Write-Host "[SUCCESS] Private endpoint created successfully!" -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "[SUCCESS] OpenAI network access rule added for private endpoint subnet" -ForegroundColor Green
} else {
    Write-Host "[WARNING] Failed to add OpenAI network rule for private endpoint (may already exist): $networkRuleResult" -ForegroundColor Yellow
}
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "[SUCCESS] OpenAI network access rule added for web app integration subnet" -ForegroundColor Green
} else {
    Write-Host "[WARNING] Failed to add OpenAI network rule for web app integration (may already exist): $webAppNetworkRuleResult" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "[WARNING] Could not retrieve private IP immediately. This is normal - DNS may still be propagating." -ForegroundColor Yellow
}

Write-Host "`n=== Private Endpoint Creation Complete ===" -ForegroundColor Green
Write-Host "[SUCCESS] Private Endpoint: $($peDetails.Name)" -ForegroundColor White
Write-Host "[SUCCESS] State: $($peDetails.State)" -ForegroundColor White
Write-Host "[SUCCESS] Location: $($peDetails.Location)" -ForegroundColor White

if (![string]::IsNullOrEmpty($privateIP) -and $privateIP -ne "null") {
    Write-Host "[SUCCESS] Private IP Address: $privateIP" -ForegroundColor White
} else {
    Write-Host "[WARNING] Private IP Address: Not yet available (DNS propagating)" -ForegroundColor Yellow
}
