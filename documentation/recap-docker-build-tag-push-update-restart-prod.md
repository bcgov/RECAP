...
This document provides the exact commands and process for updating the RECAP production webapp container image.

## Current Status Analysis

### Webapp Status
```bash
az webapp show --name "d837ad-prod-recap-webapp" --resource-group "d837ad-prod-networking" --query "{state: state, availabilityState: availabilityState, containerSettings: siteConfig.linuxFxVersion}" --output table
```

**Result**: 
- State: Running
- AvailabilityState: Normal
- Container: `DOCKER|d837adprodcontainers.azurecr.io/prod-recap-web-proxy:v2026-03-12`
...
### ACR Repository Tags
```bash
az acr repository show-tags --name d837adprodcontainers --repository prod-recap-web-proxy
# Get digest/hash for a specific tag
az acr repository show --name d837adprodcontainers --image prod-recap-web-proxy:latest
az acr repository show --name d837adprodcontainers --image prod-recap-web-proxy:stable
```
...
```bash
docker build --no-cache --pull -t prod-recap-web-proxy:latest .
```
...
docker tag prod-recap-web-proxy:latest d837adprodcontainers.azurecr.io/prod-recap-web-proxy:latest
...
# For normal operations - use stable tag  
az webapp config container set --name "d837ad-prod-recap-webapp" --resource-group "d837ad-prod-networking" --container-image-name "d837adprodcontainers.azurecr.io/prod-recap-web-proxy:stable"
```
...
- **Container Registry**: `d837adprodcontainers.azurecr.io`
- **Web App**: `d837ad-prod-recap-webapp`
- **ACR Repository**: `prod-recap-web-proxy`
...
### Application Status
- **URL**: https://d837ad-prod-recap-webapp.azurewebsites.net/
...
- **Normal Operations**: `d837adprodcontainers.azurecr.io/prod-recap-web-proxy:stable`
- **Release Deployments**: `d837adprodcontainers.azurecr.io/prod-recap-web-proxy:latest`
...
```bash
# Check local images
docker images --format "table {{.Repository}}:{{.Tag}}" | grep -E "(prod-recap-web-proxy|recap-web-proxy)"

# Run container locally for testing
docker run --rm -d --name test-container -p 8080:80 prod-recap-web-proxy:latest

# Check container logs
docker logs test-container

# Exec into container
docker run --rm --entrypoint=sh prod-recap-web-proxy:latest -c "cat /etc/nginx/nginx.conf"
```

### Azure Debugging
```bash
# Check webapp logs
az webapp log tail --name "d837ad-prod-recap-webapp" --resource-group "d837ad-prod-networking"

# Check ACR repository
az acr repository show --name d837adprodcontainers --repository prod-recap-web-proxy
...
az acr repository delete --name d837adprodcontainers --image prod-recap-web-proxy:v2026-03-12 --yes
az acr repository delete --name d837adprodcontainers --image prod-recap-web-proxy:v2026-03-12-fixed --yes
az acr repository delete --name d837adprodcontainers --image prod-recap-web-proxy:v2026-03-12-minimal --yes

# Promote stable to latest (during releases)
docker pull d837adprodcontainers.azurecr.io/prod-recap-web-proxy:stable
docker tag d837adprodcontainers.azurecr.io/prod-recap-web-proxy:stable d837adprodcontainers.azurecr.io/prod-recap-web-proxy:latest
docker push d837adprodcontainers.azurecr.io/prod-recap-web-proxy:latest

# Promote latest to stable (after successful release)
docker pull d837adprodcontainers.azurecr.io/prod-recap-web-proxy:latest
docker tag d837adprodcontainers.azurecr.io/prod-recap-web-proxy:latest d837adprodcontainers.azurecr.io/prod-recap-web-proxy:stable
docker push d837adprodcontainers.azurecr.io/prod-recap-web-proxy:stable

# Remove entire repository (use with extreme caution)
az acr repository delete --name d837adprodcontainers --repository prod-recap-web-proxy --yes
```
